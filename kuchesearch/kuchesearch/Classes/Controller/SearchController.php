<?php
namespace Kuchesearch\Kuchesearch\Controller;
...
class SearchController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
     ...
     * Search repository
     *
     * @var Kuchesearch\Kuchesearch\Domain\Repository\IndexSearchRepository
     */
    ...
    public function initialize($searchData = [])
    {
        ...
        $this->searchRepository = GeneralUtility::makeInstance(\Kuchesearch\Kuchesearch\Domain\Repository\IndexSearchRepository::class);
    ...
    {
        global $output;
        $rot = $GLOBALS['TSFE']->config['rootLine'][0]['uid'];
        $page = $this->renderPageTree($rot);
        $rootLine = $GLOBALS['TSFE']->rootLine;
        $searchData = $this->initialize($search);
        foreach ($page as $item) {

            $this->renderListPageTree($item);
            array_push($output, $item['pid']);
        }
        $output = array_unique($output);
        $listRoot = implode(",", $output);
        //$cObjectData = $this->configurationManager->getContentObject()->data;
        //\TYPO3\CMS\Core\Utility\DebugUtility::debug($searchData);
        //$page = $this->getPageTree($rot, 3);
        // Find free index uid:

        if ($searchData['typeSearchContent']) {
            if ($searchData['sword'] != '') {
                if ($searchData['typeSearchContent'] == 'pages.tt_content') {
                    ...
                    'pages.uid',
                    $queryBuilder->quoteIdentifier('tt_content.pid')
                    )
                    )*/
                        ->where(
                            $queryBuilder->expr()->like(
                                'pages.title',
                                $queryBuilder->createNamedParameter('%' . $queryBuilder->escapeLikeWildcards($searchData['sword']) . '%')
                            )
                        )
                        ->orWhere(
                            $queryBuilder->expr()->like(
                        ...
                        ->andWhere(
                            $queryBuilder->expr()->eq('pages.sys_language_uid', $queryBuilder->createNamedParameter($GLOBALS['TSFE']->sys_language_uid, \PDO::PARAM_INT))
                        )
                        ->andWhere(
                            "pages.pid in (" . $listRoot . ")"
                        )
                    ...
                    $rows1 = $queryBuilder1
                        ...
                        ->where(
                            $queryBuilder1->expr()->like(
                                'tt_content.header',
                                $queryBuilder1->createNamedParameter('%' . $queryBuilder1->escapeLikeWildcards($searchData['sword']) . '%')
                            )
                        )
                        ->andWhere(
                            $queryBuilder1->expr()->eq('tt_content.sys_language_uid', $queryBuilder1->createNamedParameter($GLOBALS['TSFE']->sys_language_uid, \PDO::PARAM_INT))
                        )
                        ->andWhere(
                            " tt_content.pid in (" . $listRoot . ")"
                        )
                        ->andWhere(
                            " tt_content.pid  <> 736"
                        )
                    ...
                    /* fetch list from tt_content*/
                    $listt = [];
                    while ($row1 = $rows1->fetch()) {
                        $list = array(
                            "uid" => $row1['uid'],
                            "doktype" => $row1['doktype'],
                            "sys_language_uid" => $row1['sys_language_uid'],
                            "pid" => $row1['pid'],
                            "title" => ($row1['title']),
                            "header" => ($row1['header']),
                            "bodytext" => strip_tags($row1['bodytext']));
                        array_push($listt, $list);
                    }
                    /*Send to view*/
                    $count = count($listt) + count($lists);
                    $this->view->assign('pagesSearch', $lists);
                    $this->view->assign('searchParams', $searchData);
                    $this->view->assign('tt_contentSearch', $listt);
                    $this->view->assign('count', $count);
                    /* the end */
                } else if ($searchData['typeSearchContent'] == 'tt_content') {
                    /* start the second part on the search select tt_content*/
                    $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tt_content');
                    $rows = $queryBuilder
                        ->select('pages.*', 'tt_content.*')
                        ->from('tt_content')
                        ->join(
                            'tt_content',
                            'pages',
                        ...
                        ->andWhere(
                            $queryBuilder->expr()->eq('tt_content.sys_language_uid', $queryBuilder->createNamedParameter($GLOBALS['TSFE']->sys_language_uid, \PDO::PARAM_INT))
                        )
                        ->andWhere(
                            "tt_content.pid in (" . $listRoot . ")"
                        )
                    ...
                    /* fetch list from tt_content*/
                    $list_tt = [];
                    while ($row1 = $rows->fetch()) {
                        //\TYPO3\CMS\Core\Utility\DebugUtility::debug($row1);
                        $j = 0;

                        if (!empty($list_tt)) {
                          
                            for ($l = 0; $l < count($output); $l++) {
                                if ($list_tt[$l]['pid'] == $row1['pid']) {
                                    $j = 1;
                                }
                            }
                        }
                         if ($j == 0) {
                        // \TYPO3\CMS\Core\Utility\DebugUtility::debug(strip_tags($row1['bodytext']));
                        //if ($row1['title'] != '' || $row1['header'] != '') {
                        ...
                        $list = array(
                            ...
                            "sys_language_uid" => $row1['sys_language_uid'],
                            "bodytext" => strip_tags($row1['bodytext']));
                        array_push($list_tt, $list);
                        //\TYPO3\CMS\Core\Utility\DebugUtility::debug("On put array");
                        /// }
                        }
                    }
                  /*  \TYPO3\CMS\Core\Utility\DebugUtility::debug($list_tt);
                  die;*/
                    $count = count($list_tt);
              //      $json_list= json_encode($list_tt);
                    $this->view->assign('searchonlyttcontent', $list_tt);
                    $this->view->assign('searchParams', $searchData);
                    $this->view->assign('count', $count);
                }
            }
    ...
    /****************************************
     * functions to getpage tree
     * ready for the output
     ***************************************/
    /* public function getPageTree(int $value=null, int $valuedepth=null)
    {
    ...
    \TYPO3\CMS\Core\Utility\DebugUtility::debug( GLOBALS['BE_USER']->getPagePermsClause());
    \TYPO3\CMS\Core\Utility\DebugUtility::debug( $tree->init());
    die;
    ...
    \TYPO3\CMS\Core\Utility\DebugUtility::debug( $tree);
     ...
     * @return mixed
     */
    public function renderPageTree($pageUid = 0)
    {
        $pid = 1;
        if ($pageUid) {
            $pid = (int) $pageUid;
        ...
        $pages = $this->t3pageRepository->getMenu($pid, $fields, $sortField, $constraints);

        foreach ($pages as &$page) {
            $subPages1 = $this->renderPageTree($page['uid']);
     ...
     * @return mixed
     */
    public function renderListPageTree(array $arr)
    {

        global $output;
        if (!empty($arr['subpages'])) {
            foreach ($arr['subpages'] as $key => $val) {
                $tt = $key;
                $output[] = $tt;
                $this->renderListPageTree($arr['subpages'][$tt]);
            }
        } else {
            $output[] = $arr['uid'];
        }
