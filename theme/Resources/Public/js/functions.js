...
(function($){
	...
	$('.sprunglink').on('click', function(){
		var href = $(this).attr('href');

		$('html, body').animate({
			scrollTop: $(href).offset().top - 10 +  'px'
		}, 600);
		return false;
	});
    ...
    if( $('.projectWrapper.list').length > 0 ){
        //GET PARAM
        $('.filter select').on('change',function(){
            var $s1="", $s2="";
            $s1 = $('.difficultySelect').val();
            //$s2 = $('.categorieSelect').val();
            if(!$s1.length){
				$('.projectItem').fadeIn(800);
            }else{
                $('.projectItem').fadeOut(300);
			}

			if( $s1.length ){
                $('.projectItem[class~="'+$s1+'"]').fadeIn(800);
			}

        });

        if(typeof(jsonPars) !== "undefined"){
			var random = jsonPars[Math.floor(Math.random() * jsonPars.length)];
			if(random.length){
				$('.randomBtn a').attr("href",random);
            }
		}
    }
