...
\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($newsRecords,"news");
    ...
    public function eventDetailAction(\Wng\WngCvciNews\Domain\Model\Event $event = NULL, $currentPage = 1, \Wng\WngCvciNews\Domain\Model\EventEmail $eventEmail = NULL) {
        ...
        if(!isset($input["tx_news_pi1"]["newEventEmail"]["firstname"]) || empty($input["tx_news_pi1"]["newEventEmail"]["firstname"])){
            $errorMessage .= LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.error.empty", "wng_cvci_news", array(LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.firstname", "wng_cvci_news")))."<br/>";
            $errorForm = TRUE;
        }
        if(!isset($input["tx_news_pi1"]["newEventEmail"]["lastname"]) || empty($input["tx_news_pi1"]["newEventEmail"]["lastname"])){
            $errorMessage .= LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.error.empty", "wng_cvci_news", array(LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.lastname", "wng_cvci_news")))."<br/>";
            $errorForm = TRUE;
        }
        if(!isset($input["tx_news_pi1"]["newEventEmail"]["email"]) || empty($input["tx_news_pi1"]["newEventEmail"]["email"])){
            $errorMessage .= LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.error.empty", "wng_cvci_news", array(LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.email", "wng_cvci_news")))."<br/>";
            $errorForm = TRUE;
        } elseif(!filter_var($input["tx_news_pi1"]["newEventEmail"]["email"], FILTER_VALIDATE_EMAIL)){
            $errorMessage .= LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.error.email", "wng_cvci_news")."<br/>";
            $errorForm = TRUE;
        }
        if(!$GLOBALS['TSFE']->fe_user->user['uid']) {
            if(!isset($input["g-recaptcha-response"]) || empty($input["g-recaptcha-response"])){
                $errorMessage .= LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.error.empty", "wng_cvci_news", array(LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.captcha", "wng_cvci_news")))."<br/>";
                $errorForm = TRUE;
            } elseif(!$this->isValidCaptcha()){
                $errorMessage .= LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.error.captcha", "wng_cvci_news")."<br/>";
                $errorForm = TRUE;
            }
        }
        ...
        if(isset($input["tx_news_pi1"]["newEventEmail"]) && !empty($input["tx_news_pi1"]["newEventEmail"])) {
            ...
            if(!$errorForm){
                ...
                $mailEvent->setSubject(LocalizationUtility::translate("email.title", "wng_cvci_news", array($event->getTitle())));
               ...
               // $mailEvent->send();
                ...
                $mailEventNotification->setSubject(LocalizationUtility::translate("email.title", "wng_cvci_news", array($event->getTitle())));
                $mailEventNotification->setBody($this->buildEventNotificationBody($event, $input), 'text/html');
                // Envoi
                //$mailEventNotification->send();

                $this->view->assign("validMessage", LocalizationUtility::translate("tx_wngcvcinews_domain_model_eventemail.valid.message", "wng_cvci_news"));
    ...
    protected function buildEventInvitationBody(\Wng\WngCvciNews\Domain\Model\Event $event) {
        $body = LocalizationUtility::translate("email.header", "wng_cvci_news")."\n\n";
        $body .= LocalizationUtility::translate("email.message", "wng_cvci_news", array($event->getTitle(), strftime('%d %B %Y', $event->getEventStartDate()->getTimestamp()), strftime('%H:%M', $event->getEventStartTime()->getTimestamp())))."\n";
        $body .= "\n";
        $body .= LocalizationUtility::translate("email.waiting", "wng_cvci_news")."\n";
        $body .= "\n";
        $body .= LocalizationUtility::translate("email.sign", "wng_cvci_news")."\n\n";
    ...
    protected function buildEventNotificationBody(\Wng\WngCvciNews\Domain\Model\Event $event = NULL, $input = array()) {
        $body = LocalizationUtility::translate("email.message2", "wng_cvci_news", array($event->getTitle()))."<br>";
        $body .= "<br>";

        // Inscrits à l'événement
        $body .= "<b>".LocalizationUtility::translate("email.event", "wng_cvci_news").":</b> ";
        ...
        $body .= "<b>".LocalizationUtility::translate("email.name", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["firstname"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.lastname", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["lastname"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.company", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["company"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.jobfield", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["jobfield"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.address", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["address"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.zip", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["zip"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.city", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["city"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.phone", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["phone"]."<br> ";

        $body .= "<b>".LocalizationUtility::translate("email.mobile", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["mobile"]."<br> ";

        if($input["tx_news_pi1"]["newEventEmail"]["aperitif"] == 1) {
            $body .= "<b>".LocalizationUtility::translate("email.aperitif", "wng_cvci_news").":</b> ";
            $body .= $input["tx_news_pi1"]["newEventEmail"]["aperitif"]."<br> ";
        }

        $body .= "<b>".LocalizationUtility::translate("email.comments", "wng_cvci_news").":</b> ";
        $body .= $input["tx_news_pi1"]["newEventEmail"]["comments"]."<br> ";

        // Email de la personne inscrite
        $body .= "<b>".LocalizationUtility::translate("email.notifications", "wng_cvci_news").":</b> ";
