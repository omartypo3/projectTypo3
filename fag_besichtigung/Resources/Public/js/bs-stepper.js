...
if ((window.location.href.indexOf("unsuccessful%5D=true") != -1) || (window.location.href.indexOf("unsuccessful]=true") != -1)) {
    ...
    getLocalStorage();
    var target1 = document.querySelector('#powermail_field_anrede_01_2_11.powermail_radio.black-label');
    var target2 = document.querySelector('#powermail_field_anrede_01_2_12.powermail_radio.black-label');
    target1.checked = "true";

    if (localStorage.getItem("second_radio") === 'true') {
...
function myLocalStorage() {
    ...
    localStorage.setItem('first_radio', document.querySelector('#powermail_field_anrede_01_2_11').checked);
    localStorage.setItem('second_radio', document.querySelector('#powermail_field_anrede_01_2_12').checked);
}
...
function dateBlocActive() {
    ...
    checkedBtn.forEach(function (btn) {
        btn.addEventListener('change', function (e) {
            ...
            var myTarget = btn.parentElement.parentElement.parentElement.parentElement.parentElement.id;

            if (btn.getAttribute('data-date').indexOf('-') > -1) {
                var d1 = new Date(btn.getAttribute('data-date'));
            } else {
                var d1 = new Date(myTarget);
            }
            if (btn.getAttribute('data-gruppentyp') != 1) {
                var d1Days = (d1.getTime()) / (1000 * 3600 * 24),
                    d2 = new Date(),
                    d2days = (d2.getTime()) / (1000 * 3600 * 24),
                    DaysDiff = parseInt(d2days) - parseInt(d1Days);
                ...
                var datetime = new Date(btn.getAttribute('data-date')+ " "+ btn.getAttribute('data-start')),
                    d1Days = (datetime.getTime() / 60000),
                    d2 = new Date(),
                    d2days = (d2.getTime()/ 60000),
                    DaysDiff = parseInt(d2days) - parseInt(d1Days);
                  if ( DaysDiff == 0 ) {
                    show();
                } else {
                    ...
                    var swiper = new Swiper('.swiper-container', {
                        ...
                        breakpoints: {
                            768: {
                                ...
                                spaceBetween: -40,
                                centeredSlides: false,
                                initialSlide: new Date().getMonth() - 2
                            },
                            480: {
                                slidesPerView: 4,
                                spaceBetween: -40,
...
function filterSelection(c) {
    ...
    document.querySelectorAll('.show').forEach(function (sh) {
        ...
        sh.firstElementChild.firstElementChild.firstElementChild.lastElementChild.classList.remove('reg');
    })
    document.querySelector('.show').lastElementChild.classList.add('in');
    document.querySelector('.show').lastElementChild.style.height = '100%';
    document.querySelector('.show').firstElementChild.firstElementChild.firstElementChild.lastElementChild.classList.add('reg');
...
function initArrow() {
    document.querySelectorAll('.show').forEach(function (sh) {
        sh.addEventListener('click', function () {
            if (sh.firstElementChild.firstElementChild.firstElementChild.lastElementChild.classList.contains('reg')) {
                sh.firstElementChild.firstElementChild.firstElementChild.lastElementChild.classList.remove('reg');
            } else {
                sh.firstElementChild.firstElementChild.firstElementChild.lastElementChild.classList.add('reg');
            }
...
function show() {
    toast.create({
        title: 'Das ausgewählte Datum wurde schon ausgebucht',
        text: ''
    });
    document.querySelector('.second-step-btn').setAttribute('onclick', 'window.location.reload()');
