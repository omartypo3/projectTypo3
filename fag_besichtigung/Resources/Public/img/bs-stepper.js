...
function eventListeners() {
    ...
    //  document.querySelectorAll('.filterDiv').forEach(function(filtredDiv){
    //     filtredDiv.addEventListener('click',function(){
    //         filtredDiv.firstElementChild.nextElementSibling.classList.toggle('in');
    //     })
    // })
...
function initFilter() {
    ...
    activeFilterBtn.forEach(function (btn) {
        if (btn.getAttribute('onclick').indexOf('filterSelection(' + actualMonthAndYear + ')') > -1) {
            btn.classList.add('years-filtre__month--active');
        }
    ...
    activeFilterBtn.forEach(function (btn) {
        btn.addEventListener('click', function () {
            activeFilterBtn.forEach(function (btn) {
                btn.classList.remove('years-filtre__month--active');
...
function filterSelection(c) {
    ...
    for (i = 0; i < x.length; i++) {        
       ...
       if (x[i].className.indexOf(c) > -1) {           
        ...
        console.log(document.querySelector('.show'),'show');
        document.querySelector('.show').lastElementChild.classList.add('in');
